...
class CaptainManager {
    ...
    enableSsl(emailAddress: string): Promise<void> {
        const self = this
        const rootDomainName = `${CaptainConstants.configs.captainSubDomain}.${self.dataStore.getRootDomain()}`;
        ...
        return Promise.resolve()
            ...
            .then(function () {
                if (
                    ...
                    rootSslConfig.credentialsFilePath
                ) {
                    Logger.d(
                        `Enabling SSL for ${rootDomainName} using DNS-01 challenge with Cloudflare.`
                    );
                    return self.certbotManager.enableSslDns01Cloudflare(
                        rootDomainName,
                        emailAddress,
                        rootSslConfig.credentialsFilePath,
                        rootSslConfig.propagationSeconds
                    );
                } else {
                    Logger.d(
